...
运行方式（在5880服务器上）：
  ...
  python nids_train_transec_gan.py \
    ...
    --epochs 200 --batch_size 256 --device cuda --num_gpus 4
...
class TransformerEncoder(nn.Module):
    """Transformer 编码器"""
    def __init__(self, input_dim, d_model=128, nhead=8, num_layers=4, seq_len=32):
        super().__init__()
...
class Generator(nn.Module):
    """生成器：噪声 + 类别条件 → 合成序列"""
    def __init__(self, latent_dim, num_classes, pca_dim, seq_len=32):
        super().__init__()
        self.seq_len = seq_len
        self.noise_linear = nn.Linear(latent_dim + num_classes, 128)
        self.transformer = TransformerEncoder(input_dim=128, d_model=128, seq_len=seq_len)
...
class Discriminator(nn.Module):
    """判别器：序列 → 真/假分数 + 分类"""
    def __init__(self, pca_dim, num_classes, seq_len=32):
        super().__init__()
        self.transformer = TransformerEncoder(input_dim=pca_dim, d_model=128, seq_len=seq_len)
...
class TransECGANTrainer:
    def __init__(self, pca_dim, num_classes, seq_len=32, latent_dim=128,
                 lr_d=2e-4, lr_g=1e-4, device='cuda', num_gpus=1):
...
def main():
    ...
    if device == 'cuda' and not torch.cuda.is_available():
        ...
        logger.info(f"GPU: {gpu_name} × {gpu_count}")
